...
while(True):
    ...
    if endno==noumero:
        subprocess.run(['python','decode_from_file_and_append_to_final2.py',strnoumero,diafora])
        print("teleiosa")
        break
    subprocess.run(['python','decode_from_file_and_append_to_final2.py',strnoumero])
   # noumero+=1
    noumero+=1
    strnoumero=str(noumero)
