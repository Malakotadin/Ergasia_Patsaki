...
def lista_posostron_finder(demo_string):
    ...
    return lista_arhikon_pososton,lista_monadikon_xarakthron
...
def encoder_wrapper(string_to_use,i,dictionary_apothikeuesis):
    ...
    dictionary_apothikeuesis[i][0],dictionary_apothikeuesis[i][1]=lista_posostron_finder(string_to_use)#0λιστα αρχικων ποσοστον , 1 λιστα μοναδικων χαρακτηρων
...
counter1=0
counter2=0
for i in range(0,len(dictionary_apothikeuesis)-1):

    if dictionary_apothikeuesis[i][1]==dictionary_apothikeuesis[i+1][1]:
        #print("exit 1 ")#
        #exit()
        counter1+=1
        #breakpoint()

    if dictionary_apothikeuesis[i][0]==dictionary_apothikeuesis[i+1][0]:
   # print("exit 0 ")# και αυτες οι τρεις μαλακιες ειναι ιδιες ΓΑΜΩΩΩΩΩΩΩΩΩ
    #exit()
        counter0+=1
    if dictionary_apothikeuesis[i][2]==dictionary_apothikeuesis[i+1][2]:
        #print("balls")# κανω encode το ιδιο πραγμα ξανα και ξανα , αλλα γιατι 
        #exit()
        counter2+=1

print(counter0,counter1,counter2)
print(len(dictionary_apothikeuesis))
#exit()
counter_foron=0
...
f= open('final_photo.jpg','br+')
f.write(bytes.fromhex(teliko_string))
f.close()
#else:
    #breakpoint()
end2=time.time()
